...
class QAgent(BaseAgent):
    ...
    def max_action_q(self, observation:Observation) -> float:
        ...
        """
        # Obtener el estado actual de la observación
        state = observation.state
        # Generar todas las acciones validas para este estado
        actions = generate_valid_actions(state)
        # Obtener el ID del estado
        state_id = self.get_state_id(state)

        # Si no hay acciones validas, no hay valor Q que devolver.
        if not actions:
            return 0.0

        # Encontrar la accion con el valor Q maximo
        ...
        # Si max_q_value nunca se actualizo devolvemos 0.0, ya que es el valor por defecto.
    ...
    def select_action(self, observation:Observation, testing=False) -> tuple:
        ...
        """
        # Obtener el estado y generar acciones validas
        state = observation.state
        actions = generate_valid_actions(state)
        # Obtener el state_id para este estado
        state_id = self.get_state_id(state)
        
        # Si no hay acciones, devolvemos una accion aleatoria de una lista vacia
        # lo que causara un error, pero es un caso que no debería ocurrir en este entorno.
        if not actions:
            return random.choice(list(actions)), state_id

        # Logica Epsilon Greedy
        ...
        if not testing and random.uniform(0, 1) < self.current_epsilon:
            # Etapa de exploracion: Elegimos una acción aleatoria
            action = random.choice(list(actions))
        else:
            # Etapa de explotacion: Elegimos la mejor accion
            max_q_value = -np.inf
            best_action = None
            
            # Forma aleatoria si varias tienen el mismo valor Q maximo.
        ...
        # Inicializar el valor Q a 0 si el par (state_id, action) no existe
        if (state_id, action) not in self.q_values:
            self.q_values[state_id, action] = 0.0

        # Devolver la acción seleccionada y el state id
